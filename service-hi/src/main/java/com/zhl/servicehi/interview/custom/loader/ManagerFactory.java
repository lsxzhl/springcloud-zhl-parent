...
public class ManagerFactory {
     ...
     * 实现热加载的类的全名称（包名+类名）
     */
    public static final String MY_MANAGER = "com.zhl.servicehi.interview.custom.loader.CustomManager";

    public static BaseManager getManager(String className) {
        File loadFile = new File(CLASS_PATH + className.replaceAll("\\.", "/") + ".class");
        //获取最后一次修改时间
        long lastModified = loadFile.lastModified();
        System.out.println("当前类的最后修改时间：" + lastModified);
        // loadTimeMap 不包含 ClassName 为 key 的信息  证明这个类没有被加载 要加载到 JVM
        if (loadTimeMap.get(className) == null) {
            load(className,lastModified);
        } //加载类的时间戳变化了  我们同样要重新加载到这个类到 JVM
